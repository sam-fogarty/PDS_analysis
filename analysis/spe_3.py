...
def main(filepath):
    ...
    hist_range = (-100, 500)
    hist = np.histogram(integrals, bins = round((1/10)*(max(integrals)-min(integrals))))
    hist_bins, hist_bincenters = hist[0], hist[1][:-1]
    x_values = np.arange(hist_range[0], hist_range[1], 1)
    if do_gaussian_fit:

        peaks = signal.find_peaks(hist[0], height = 0.2*max(hist[0]))
        print(peaks)
        data = hist[0][:(peaks[0][2]+round(0.1*abs(hist[1][peaks[0][0]]-hist[1][peaks[0][1]])))]
        fit, error = curve_fit(Gaussian3, hist[1][:len(data)], data, p0=[hist[1][peaks[0][0]], hist[1][peaks[0][1]], hist[1][peaks[0][2]], 25, 25, 25, hist[0][peaks[0][0]], hist[0][peaks[0][1]], hist[0][peaks[0][2]]])
